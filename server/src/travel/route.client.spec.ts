...
describe("RouteClient", () => {
  ...
  it("rejects a non-object external response", async () => {
    jest.spyOn(global, "fetch").mockResolvedValue({
      ok: true,
      json: async () => null,
    } as Response);
    ...
    await expect(client().routeFor(from, to))
      .rejects
      .toEqual(new RouteRequestError("出発地と到着地の道路ルートが見つかりません。"));
  });
  ...
  it.each([
    { duration: -1, distance: 450_000 },
    { duration: 4_200, distance: -1 },
  ])("rejects routes with negative duration or distance", async ({ duration, distance }) => {
    jest.spyOn(global, "fetch").mockResolvedValue({
      ok: true,
      json: async () => ({ code: "Ok", routes: [{ duration, distance }] }),
    } as Response);
