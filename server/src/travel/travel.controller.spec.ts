...
describe("TravelController", () => {
  ...
  it("rejects an invalid departure window", async () => {
    const service = { bestDeparture: jest.fn().mockRejectedValue(new TravelInputError("出発可能時間の範囲が正しくありません。")) } as unknown as TravelService;
    const controller = new TravelController(service);
    const request = { user: { id: 99n } } as AuthenticatedRequest;
    ...
    try {
      ...
      expect((error as HttpException).getResponse()).toEqual({ errors: ["出発可能時間の範囲が正しくありません。"] });
      expect(service.bestDeparture).toHaveBeenCalled();
    }
  });

  it("does not expose unexpected service errors", async () => {
    const error = new Error("database connection details");
    const service = { plan: jest.fn().mockRejectedValue(error) } as unknown as TravelService;
    const controller = new TravelController(service);
    const request = { user: { id: 99n } } as AuthenticatedRequest;

    await expect(controller.plan(request, "1", "2")).rejects.toBe(error);
  });
});
