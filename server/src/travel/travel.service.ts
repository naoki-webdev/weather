...
export class TravelService {
  ...
  async bestDeparture(
    ...
    if (departureWindowStart >= departureWindowEnd) {
      throw new TravelInputError("出発可能時間の範囲が正しくありません。");
    }
    if (!Number.isInteger(intervalMinutes) || intervalMinutes < 5 || intervalMinutes > 60) {
      throw new TravelInputError("候補の間隔は5分から60分の間で指定してください。");
    }
    ...
    if (candidateCount > 25) {
      throw new TravelInputError("出発可能時間は候補が25件以内になるよう指定してください。");
    }
  ...
  private async travelContext(userId: bigint, fromId: bigint, toId: bigint) {
    if (fromId === toId) throw new TravelInputError("出発地と到着地には別の都市を選択してください。");
