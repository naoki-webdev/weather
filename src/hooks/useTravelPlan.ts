...
export function useTravelPlan(selectedIds: number[]) {
  ...
  const planTravel = useCallback(async (departureAt?: string) => {
    ...
    try {
      ...
      if (requestSequence === travelRequestSequence.current) setTravelPlan(response);
    } catch (requestError) {
  ...
  const planBestDeparture = useCallback(async (windowStart: string, windowEnd: string, intervalMinutes = 15) => {
    ...
    try {
      ...
      if (requestSequence === bestDepartureRequestSequence.current) setBestDeparturePlan(response);
    } catch (requestError) {
