...
export default function ComparisonTable({ cities, leaderId, historyPeriodDays, loading, onRemove }: ComparisonTableProps) {
  ...
  return <>
    <Stack direction="row" spacing={0.5} alignItems="center" justifyContent="space-between">
      ...
      <Typography variant="subtitle2">{t("weather.comparison.history_label", { days: historyPeriodDays })}</Typography>
      <HelpTooltip title={t("weather.comparison.history_hint", { days: historyPeriodDays })} label={t("weather.comparison.history_help_label")} />
      </Stack>
      <Typography variant="caption" color="text.secondary" sx={{ display: { xs: "block", sm: "none" } }}>{t("weather.comparison.scroll_hint")}</Typography>
    </Stack>
    <TableContainer sx={{ maxWidth: "100%", overflowX: "auto" }} tabIndex={0} aria-label={t("weather.comparison.title")}>
