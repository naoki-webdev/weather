  ...
  const content = <Stack spacing={1.25}>
        ...
        <Stack direction={{ xs: "column", sm: "row" }} spacing={1.25} alignItems={{ xs: "stretch", sm: "center" }}>
          <TextField
            ...
            inputProps={{ step: 300 }}
          />
          <Button type="submit" variant="contained" disabled={loading || !departureAt} aria-busy={loading} sx={{ whiteSpace: "nowrap" }}>
            {loading ? t("weather.travel.loading") : t("weather.travel.action")}
          </Button>
        </Stack>
      </Stack>
  return compact ? <form onSubmit={submit}>{content}</form> : <Paper component="form" variant="outlined" onSubmit={submit} sx={{ p: 2, borderRadius: 2, backgroundColor: "rgba(255,255,255,0.7)" }}>{content}</Paper>;
}
