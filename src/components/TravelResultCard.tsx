...
function recommendationColor(code: TravelPlan["recommendation"]["code"]) {
  if (code === "umbrella" || code === "caution") return "warning.main";
  if (code === "clear") return "success.main";
      ...
      <Stack spacing={1.25}>
        <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" spacing={1}>
          ...
          <Stack direction="row" spacing={1} alignItems="center">
            <Typography variant="body2" color={recommendationColor(plan.recommendation.code)} fontWeight={850}>{t(`weather.travel.recommendation.${plan.recommendation.code}`)}</Typography>
        ...
        {plan.arrival_weather && <Stack spacing={0.25}>
          <Typography variant="body2" fontWeight={800}>{t("weather.travel.arrival_weather")}</Typography>
          <Typography variant="body2">{formatTemperature(plan.arrival_weather.temperature)} {weatherCodeLabel(plan.arrival_weather.weather_code)}</Typography>
          <Typography variant="body2" color="text.secondary">{t("weather.travel.rain_probability", { value: formatPercent(plan.arrival_weather.precipitation_probability) })} / {t("weather.travel.wind", { value: formatNumber(plan.arrival_weather.wind_speed, " km/h") })}</Typography>
        </Stack>}
      </Stack>
    </Box>
  );
