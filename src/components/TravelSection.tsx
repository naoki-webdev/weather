    ...
    <Stack spacing={1.5}>
      <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" spacing={1}>
        <div>
          ...
          <Typography variant="body2" color="text.secondary">{t("weather.travel.section_description", { from: from.name, to: to.name })}</Typography>
        </div>
        <Typography variant="body2" fontWeight={800}>{from.name} → {to.name}</Typography>
      </Stack>
      <Typography variant="caption" color="text.secondary">{t("weather.travel.timezone_hint", { timezone: from.timezone })} ・ {t("weather.travel.arrival_timezone_hint", { timezone: to.timezone })}</Typography>
      <FormControl>
      ...
      {mode === "single" && travelError && <Alert severity="error">{travelError}</Alert>}
