...
import { compareCities, searchCities } from "./cityRequests";
...
describe("city requests", () => {
  ...
  test("passes an AbortSignal to the city search request", async () => {
    const controller = new AbortController();
    vi.spyOn(global, "fetch").mockResolvedValue({
      ok: true,
      json: async () => ({ results: [] }),
    ...
    await searchCities("Tokyo", controller.signal);

    expect(fetch).toHaveBeenCalledWith(
      expect.stringContaining("query=Tokyo"),
      expect.objectContaining({ signal: controller.signal }),
    );
  });
});
