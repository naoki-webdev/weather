...
export async function searchCities(query: string): Promise<CitySearchResult[]> {
  ...
  const response = await requestJson<{ results: CitySearchResult[] }>(`${API_BASE_URL}/api/cities/search?${searchParams.toString()}`);
...
export async function syncCity(id: number): Promise<City> {
  return requestJson<City>(`${API_BASE_URL}/api/cities/${id}/sync`, { method: "POST" });
}
...
export async function fetchWeatherPreference(): Promise<WeatherPreference> {
